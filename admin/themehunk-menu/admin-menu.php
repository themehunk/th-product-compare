...
if (!function_exists('themehunk_admin_menu')) {
  include_once( ABSPATH . 'wp-admin/includes/plugin-install.php' );
    ...
    function themehunk_admin_menu(){
            add_menu_page(__('ThemeHunk', 'th-product-compare'), __('ThemeHunk', 'th-product-compare-pro'), 'manage_options', 'themehunk-plugins', 'themehunk_plugins',  THEMEHUNK_PURL . '/th-option/assets/images/themehunk-icon.svg', 59);
        }
...
function admin_scripts( $hook ) {
    ...
    if ($hook === 'toplevel_page_themehunk-plugins'  ) {
      ...
      wp_enqueue_style( 'themehunk-plugin-css', THEMEHUNK_PURL . '/th-option/assets/css/started.css', array(), TH_PRODUCT_VERSION );

      wp_enqueue_script('themehunk-plugin-js', THEMEHUNK_PURL . '/th-option/assets/js/th-options.js',array( 'jquery', 'updates' ), TH_PRODUCT_VERSION, true);
